  ...
  return (
    <>
      ...
      <section className="relative min-h-[520px] flex items-center overflow-hidden">
        ...
        <div className="absolute inset-0 bg-gradient-to-r from-[rgba(20,18,14,0.88)] via-[rgba(20,18,14,0.55)] to-[rgba(20,18,14,0.2)]" />
        <div className="relative z-10 px-14 py-16 max-w-[580px]">
          <p className="font-sans text-[10px] tracking-[0.25em] uppercase text-[#C4A882] mb-4">Grünerløkka · Oslo · Hesselbergs gate 7</p>
          <h1 className="text-[40px] text-white font-normal leading-tight mb-3">Vi tar vare på<br />plaggene du er <em className="italic text-[#C4A882]">glad i</em></h1>
          <p className="font-sans text-[13px] text-white/60 mb-8 leading-loose">Kjemisk rens · Skreddertjenester · Skjorteservice<br />Rask og pålitelig — klar på 24 timer.</p>
          <div className="flex gap-3 flex-wrap">
            <Link href="/bestilling" className="bg-[#C4A882] text-[#1A1A18] px-7 py-3 font-sans text-[11px] tracking-widest uppercase font-bold">Bestill henting</Link>
            <Link href="/prisliste" className="border border-white/35 text-white/80 px-7 py-3 font-sans text-[11px] tracking-widest uppercase">Se prisliste</Link>
          </div>
        </div>
      </section>

      {/* INFO STRIP */}
      <div className="grid grid-cols-4 bg-[#22221F] border-t border-white/[0.06]">
      ...
      <div className="grid grid-cols-3 gap-[3px]" style={{gridTemplateRows:'260px 260px'}}>
        <div className="relative overflow-hidden row-span-2 group">
          ...
          <div className="absolute bottom-0 left-0 right-0 p-5 bg-gradient-to-t from-[rgba(20,18,14,0.75)] to-transparent font-sans text-[11px] tracking-widest uppercase text-white/85">Kjemisk rens</div>
        </div>
        {[['https://images.unsplash.com/photo-1512436991641-6745cdb1723f?w=800&q=80','Skjorteservice'],['https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=800&q=80','Skreddertjenester'],['https://images.unsplash.com/photo-1490481651871-ab68de25d43d?w=800&q=80','Delikate plagg'],['https://images.unsplash.com/photo-1469334031218-e382a71b716b?w=800&q=80','Bunad & selskapstøy']].map(([src,label]) => (
          <div key={label} className="relative overflow-hidden group">
            ...
            <div className="absolute bottom-0 left-0 right-0 p-5 bg-gradient-to-t from-[rgba(20,18,14,0.75)] to-transparent font-sans text-[11px] tracking-widest uppercase text-white/85">{label}</div>
          </div>
        ))}
      </div>
      ...
      <section className="py-[72px] px-10 max-w-[960px] mx-auto">
        <p className="font-sans text-[10px] tracking-[0.22em] uppercase text-[#8B7355] mb-2 text-center">Hva vi tilbyr</p>
        <h2 className="text-[30px] font-normal text-center mb-2">Våre tjenester</h2>
        <p className="font-sans text-[13px] text-[#7A7468] text-center max-w-[500px] mx-auto mb-12 leading-loose">Siden 1995 har vi levert tjenester av høy kvalitet på Grünerløkka. Vi behandler hvert plagg som vårt eget.</p>
        <div className="grid grid-cols-3 gap-[3px] bg-[#E5E0D5] border border-[#E5E0D5]">
          {SERVICES.map(({n,title,desc,price,href}) => (
            <Link key={n} href={href} className="bg-white p-8 hover:bg-[#FAF8F3] transition-colors block">
              <div className="font-sans text-[10px] tracking-widest text-[#C4A882] mb-3">{n}</div>
              <h3 className="text-[16px] font-normal mb-2 tracking-wide">{title}</h3>
              <p className="font-sans text-[12px] text-[#7A7468] leading-relaxed mb-3">{desc}</p>
              <div className="font-sans text-[11px] text-[#5C6E50] tracking-wide uppercase border-t border-[#E5E0D5] pt-3">{price} →</div>
            </Link>
          ))}
        </div>
      </section>
      ...
      {/* DARK FEATURES */}
      <section className="bg-[#1A1A18] py-[72px] px-10">
        <div className="max-w-[900px] mx-auto grid grid-cols-3 gap-12">
          {[['Praktisk','Henting & levering','Vi henter og leverer direkte til deg. Bestill på nett — vi fikser resten.'],['Garanti','Alltid fornøyd','Vi er ikke fornøyd med arbeidet vårt før du er det. Fullt ansvar for hvert plagg.'],['Erfaring','30 år i bransjen','Thomas Rens og Vask har betjent Grünerløkka siden 1995. Din tillit er vår viktigste verdi.']].map(([n,title,body]) => (
            <div key={n} className="border-t border-white/10 pt-6">
              <div className="font-sans text-[10px] tracking-widest uppercase text-[#C4A882] mb-3">{n}</div>
      ...
      <section className="grid grid-cols-2 min-h-[420px]">
        ...
        <div className="bg-[#FAF8F3] px-14 py-16 flex flex-col justify-center">
          <p className="font-sans text-[10px] tracking-[0.22em] uppercase text-[#8B7355] mb-3">Om oss</p>
          <h2 className="text-[28px] font-normal mb-4 leading-snug">Et renseri bygget<br />på faglig stolthet</h2>
          <p className="font-sans text-[13px] text-[#7A7468] leading-loose mb-3">Thomas Rens og Vask har holdt til på Hesselbergs gate siden 1995. Et lokalt renseri med dyp respekt for hvert plagg vi tar imot.</p>
          <p className="font-sans text-[13px] text-[#7A7468] leading-loose mb-4">Faglig stolthet og teknisk innsikt sikrer at kjemisk rens alltid utføres etter de strengeste bransjestandarder.</p>
          <Link href="/bestilling" className="inline-block border border-[#1A1A18] text-[#1A1A18] px-6 py-3 font-sans text-[11px] tracking-widest uppercase hover:bg-[#1A1A18] hover:text-white transition-all w-fit">Book time nå</Link>
        </div>
      </section>
      ...
      <section className="py-[72px] px-10 bg-white">
        <div className="max-w-[900px] mx-auto">
          <p className="font-sans text-[10px] tracking-[0.22em] uppercase text-[#8B7355] mb-3 text-center">Hva kundene sier</p>
          <div className="flex items-baseline gap-4 justify-center mb-2">
            <span className="font-serif text-[54px] leading-none">4,1</span>
            <span className="text-[22px] text-[#8B7355] tracking-[3px]">★★★★☆</span>
          </div>
          <p className="font-sans text-[12px] text-[#7A7468] text-center mb-11">35 anmeldelser på Google</p>
          <div className="grid grid-cols-3 gap-5">
            {REVIEWS.map(({stars,text,author},i) => (
              <div key={i} className="bg-[#FAF8F3] p-7">
                <div className="text-[#8B7355] text-[13px] tracking-[2px] mb-2">{'★'.repeat(stars)}</div>
                <p className="font-serif text-[13px] leading-loose italic mb-4">"{text}"</p>
                <p className="font-sans text-[11px] text-[#7A7468] tracking-wide">— {author}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="grid grid-cols-2 min-h-[380px]">
        ...
        <div className="bg-[#22221F] px-14 py-14 flex flex-col justify-center">
          <p className="font-sans text-[10px] tracking-[0.22em] uppercase text-[#C4A882] mb-3">Åpningstider</p>
          <h2 className="text-[26px] text-white font-normal mb-7">Kom og besøk oss</h2>
          {HOURS.map((day,i) => (
            <div key={day} className={`flex justify-between py-2 border-b border-white/[0.07] font-sans text-[13px] ${i===todayIndex?'text-[#C4A882] font-bold':''}`}>
              <span className={i===todayIndex?'text-[#C4A882]':'text-white/45'}>{day}{i===todayIndex?' ← i dag':''}</span>
              <span className={i===todayIndex?'text-[#C4A882]':'text-white/85'}>{TIMES[i]}</span>
            </div>
          ))}
        </div>
      </section>
